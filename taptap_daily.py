...
HTML_DIR     = os.path.join(DATA_DIR, "html_report_daily")
...
def shorten_url(long_url):
    """用 is.gd 缩短网址，失败则返回原链接"""
    try:
        api = f"https://is.gd/create.php?format=simple&url={urllib.parse.quote(long_url, safe='')}"
...
# ══════════════════════════════════════════════
# HTML & Chat Summary (simplified for daily)
# ══════════════════════════════════════════════
CSS = """
...
.header{background:linear-gradient(135deg,#0f3460,#16213e);color:#fff;padding:40px 30px;border-radius:12px;margin-bottom:24px}
...
def summarize_items(items, label="问题"):
    """Generate content summary for bug/complain items, extracting titles and key points."""
    if not items:
        return ""
    lines = []
...
<body>
<div class="container">
<div class="header" style="background:linear-gradient(135deg,#0f3460,#16213e)">
...
<div class="meta">📅 {date_str} &nbsp;|&nbsp; 🕐 生成: {datetime.now().strftime('%Y-%m-%d %H:%M')} &nbsp;|&nbsp; 📡 数据来源: TapTap</div>
    ...
    if suggest_items:
        H.append(f'<p><strong>💡 建议聚焦：</strong>玩家主要关注英雄/角色平衡、装备机制优化、关卡难度等方向。共 {len(suggest_items)} 条建议帖。</p>')
    if bug_items:
...
def generate_chat_summary(all_items, date_str, report_url=""):
    """Generate daily chat summary for Feishu push."""
    reviews = [i for i in all_items if i["source"] == "review"]
    posts   = [i for i in all_items if i["source"] == "forum"]
    total = len(all_items)
    cat_map = defaultdict(list)
    ...
    L = []
    L.append(f"📊 **{GAME_NAME} TapTap 舆情日报**")
    L.append(f"📅 {date_str}")
    L.append("")
    L.append("---")
    L.append("")
    L.append("📋 **本日舆情概括**")
    L.append("")
    L.append(f"本日共产生 **{total}** 条内容（{len(reviews)}条评分 + {len(posts)}条论坛帖子），玩家回复 **{total_c}** 条，嵌套回复 **{total_n}** 条，互动总量 **{total_interact}** 条。")
    L.append("")
    ...
    if suggest_items:
        L.append(f"💡 **建议聚焦**：共 {len(suggest_items)} 条，关注英雄平衡、装备机制、关卡难度等方向。")
        L.append("")
    if bug_items:
        L.append(f"🐛 **BUG聚焦**：报告了 **{len(bug_items)}** 个问题：")
        L.append("")
        L.append(summarize_items(bug_items, "BUG"))
        L.append("")
    if complain_items:
        L.append(f"😤 **吐槽聚焦**：共 {len(complain_items)} 条负面内容：")
        L.append("")
        L.append(summarize_items(complain_items, "吐槽"))
        L.append("")
    if topic_counter:
        top_3 = topic_counter.most_common(3)
        L.append(f'🔥 本日热点：{"、".join([f"{t}({c}次)" for t,c in top_3])}')
        L.append("")

    # TOP10 list
    hot_posts = sorted([p for p in posts if p.get("comments")], key=lambda x: len(x["comments"]), reverse=True)
    top10 = hot_posts[:10]
    if top10:
        L.append("---")
        L.append("")
        ...
        for idx, p in enumerate(top10):
            ...
            L.append(f"{idx+1}. [{title}]({short_url}) — {p['author']} | {len(p['comments'])}回复")
        L.append("")

        L.append("---")
        L.append("")
        # TOP3 comments
        for idx, p in enumerate(top10[:3]):
            title = (p.get("title","") or p.get("body","")[:30])[:30]
            L.append(f"🔥 **TOP{idx+1} 精选评论：{title}**")
            L.append("")
            for c in p.get("comments", [])[:2]:
                L.append(f"> {c['author']}：{c['text'][:120]}")
            L.append("")

    L.append("---")
    L.append("")
    L.append(f"📄 **完整日报**：{report_url or '即将部署'}")
...
def main():
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write(generate_chat_summary(all_items, DATE, report_url="__CLOUDSTUDIO_URL__"))
    log(f"[保存] 推送摘要 -> chat_summary_daily_latest.txt")
